...
class ExampleApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def calculate(self):  
        ...
        if self.lineEdit.text() == "":
            ...
            self.source = self.lineEdit.text()
            
        
        self.listWidget.clear()
        self.listWidget.addItem('Длина блока кодирования: ' +str(CHUNK_LENGTH))
        
        self.listWidget.addItem('Контрольные биты: '+str(CHECK_BITS))
            
        encoded = encode(self.source)
        self.listWidget.addItem('Закодированные данные: \n'+str(encoded))
            
        decoded = decode(encoded)
        self.listWidget.addItem('Результат декодирования: '+str(decoded))
            
        encoded_with_error = set_errors(encoded)
        self.listWidget.addItem('Допускаем ошибки в закодированных данных:\n'+str(encoded_with_error))
        
        diff_index_list = get_diff_index_list(encoded, encoded_with_error)
        self.listWidget.addItem('Допущены ошибки в битах: '+', '.join(list(map(str,diff_index_list))))
        
        decoded = decode(encoded_with_error, fix_errors=False)
        self.listWidget.addItem('Результат декодирования ошибочных данных без исправления ошибок: '+str(decoded))
        
        decoded = decode(encoded_with_error)
        self.listWidget.addItem('Результат декодирования ошибочных данных с исправлением ошибок: '+str(decoded))
